...
import com.vision.smarthomeapi.sqlutil.exceptions.ParseConfigurationFileException;
...
public class LitePalParser {
    ...
    private InputStream getConfigInputStream() throws IOException {
        ...
        OutPutMessage.LogCatInfo("数据表格调用", Arrays.toString(fileNames) + "!!!!!!!!!!!");
        if (fileNames != null && fileNames.length > 0) {
            for (String fileName : fileNames) {
                OutPutMessage.LogCatInfo("数据表格调用", fileName + "!!!!!!!!!!!");
